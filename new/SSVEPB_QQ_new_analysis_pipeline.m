function SSVEPB_QQ_new_analysis_pipeline()
    ...
    preidx = {[1,5:8,11:13,15,17,18,21:23,25:27]; ...   % QQ_A
        [9,10,14,16,19,20,24,27]; ...             % QQ_B
        [32]; % QQ_A new
        [34:40]; % QQ_B new
        [3:15]; ...                                     % DG_A
        [16:23]};                                       % DG_B

    USER_SETTINGS.EXTRACTION_DAYS         = preidx{4};     % Days for signal extraction
    USER_SETTINGS.EXTRACTION_DATA_TYPE    = 'LFP';              % 'MUA1', 'MUA2', 'LFP'
    USER_SETTINGS.EXTRACTION_BLOCKS       = {'MGv','MGnv','SG','SSGnv','SSGv','blank'};
...
function run_signal_extraction(config)
    ...
    for day_loop_idx = 1:length(config.process_params.Days)
        ...
        for session_loop_idx = 1:length(session_file_numbers)
            ...
            [session_repeat_counts, session_clustered_data] = TrialDataRearrange(config, ReFactor, neural_data_for_session(:,1:94,1:1640));
    ...
    if ismember('SSGnv', selected_blocks)
        ...
        for loc = 1:13
            for ori = 1:10
                ...
                block_data_structs.SSGnv(idx).Pattern = [];
                idx = idx + 1;
            end
        end
    end
    if ismember('SSGv', selected_blocks)
        num_ssgv_conditions = 13 * 10; % 13 locations, 18 orientations
        block_data_structs.SSGv = repmat(base_condition_template, 1, num_ssgv_conditions);
        idx = 1;
        for loc = 1:13
            for ori = 1:10
                block_data_structs.SSGv(idx).Block = 'SSGv';
                block_data_structs.SSGv(idx).Location = loc;
                block_data_structs.SSGv(idx).Target_Ori = condition(ori);
                block_data_structs.SSGv(idx).Pattern = [];
                idx = idx + 1;
            end
        end
    end
...
function [repeatnum_per_ori, ClusterData] = TrialDataRearrange(config, ReFactor, trialdata)
    ...
    ClusterData = struct(...
        ...
        'MGnv', {cell(3, 10)}, ...
        'SG',   {cell(3, 10)}, ...
        'SSGnv',{cell(3, 13, 10)}, ... % {loc,ori}=Data (QQ processing specific)
        'SSGv',{cell(3, 13, 10)}, ...
        ...
        if strcmp(currentBlock,'SSGnv')
            for loc = 1:13
                ...
                for condi = 1:length(-1:2:17)
                    ...
                    ClusterData.SSGnv{3, loc,condi} = int16([ReFactor(idx).Pattern]);
                end
            end
        elseif strcmp(currentBlock,'SSGv')
            for loc = 1:12
                ...
                for condi = 1:length(-1:2:17)
                    ...
                    % filteredData = filtfilt(b, a, single(data)')'; % 滤波
                    % filteredData = reshape(filteredData,[1,96,1640]);
                    ClusterData.SSGv{1, loc, condi} = int16(filteredData);

                    ClusterData.SSGv{2, loc,condi} = int16([ReFactor(idx).Stim_Sequence]);
                    ClusterData.SSGv{3, loc,condi} = int16([ReFactor(idx).Pattern]);
                end
            end
        elseif strcmp(currentBlock,'blank')
            idx = charIndices{i};
            ClusterData.blank = cat(1,ClusterData.blank,single(trialdata(idx,:,1:1640)));
        else
            ...
            for condi = 1:length(condition)
                ...
                data = trialdata(idx, :, t.triallength);
                filteredData = data - mean(data(:,:,1:100),3);
                % filteredData = filtfilt(b, a, single(data)')'; % 滤波
                % filteredData = reshape(filteredData,[1,96,1640]);
...
function current_block_data = update_block_data_with_session(current_block_data, session_cluster_data, selected_blocks_for_processing)
    ...
    for i = 1:length(selected_blocks_for_processing)
                    ...
                    for cond = 1:size(session_cluster_data.SSGnv, 3)
                        if ~isempty(session_cluster_data.SSGnv{1,loc, cond})
                            ...
                            if ~isempty(struct_idx)
                                ...
                                current_block_data.SSGnv(struct_idx).Pattern = cat(1, current_block_data.SSGnv(struct_idx).Pattern, session_cluster_data.SSGnv{3,loc, cond});
        ...
        elseif strcmpi(block_name, 'SSGv') % SSGnv from ClusterData is {loc, ori} cell
            if isfield(session_cluster_data, 'SSGv') && ~isempty(session_cluster_data.SSGv)
                for loc = 1:size(session_cluster_data.SSGv, 2)
                    for cond = 1:size(session_cluster_data.SSGv, 3)
                        if ~isempty(session_cluster_data.SSGv{1,loc, cond})
                            struct_idx = find([current_block_data.SSGv.Location] == loc & ...
                                [current_block_data.SSGv.Target_Ori] == condition(cond));
                            if ~isempty(struct_idx)
                                current_block_data.SSGv(struct_idx).Data = cat(1, current_block_data.SSGv(struct_idx).Data, session_cluster_data.SSGv{1, loc, cond});
                                current_block_data.SSGv(struct_idx).Pic_Ori = cat(1, current_block_data.SSGv(struct_idx).Pic_Ori, session_cluster_data.SSGv{2,loc, cond});
                                current_block_data.SSGv(struct_idx).Pattern = cat(1, current_block_data.SSGv(struct_idx).Pattern, session_cluster_data.SSGv{3,loc, cond});
                                current_block_data.SSGv(struct_idx).Target_Ori = condition(cond);
                            end
                        end
                    end
                end
    ...
    for i = 1:length(block_names)
        ...
        if ~is_data_empty
            if strcmp(current_block_name,'SSGnv')||strcmp(current_block_name,'SSGv')
                save_filename = sprintf('%sSSVEPB_%s_%s_%s_%s.mat', file_prefix, days_str, mua_lfp_str,current_block_name,location_idx);
            else
                save_filename = sprintf('%sSSVEPB_%s_%s_%s.mat', file_prefix, days_str, mua_lfp_str, current_block_name);
            end
