function SSVEP_analysis_pipeline_1000hz()
    ...
    preidx = {[1,5:8,11:13,15,17,18,21:23,25:29]; ...   % QQ_A
        [9,10,14,16,19,20,24,27,28,29]; ...             % QQ_B
        [3:15]; ...                                     % DG_A
        [16:23]};                                       % DG_B
    USER_SETTINGS.EXTRACTION_DAYS         = preidx{2}(1:8);  % Days for signal extraction
    USER_SETTINGS.EXTRACTION_DATA_TYPE    = 'MUA2';       % 'MUA1', 'MUA2', 'LFP'
    USER_SETTINGS.EXTRACTION_BLOCKS       = {'MGv','MGnv','SG'};
...
function config = get_experiment_config(USER_SETTINGS)
    ...
    config.TrialDataParams.triallength     = 1:3000;
...
function run_signal_extraction(config)
    ...
    for day_loop_idx = 1:length(config.process_params.Days)
        ...
        if config.is_DG
            session_file_numbers = [sessionIdx_data{3:5, current_day_val}]; % 这个位置要记得改一下
        ...
        for session_loop_idx = 1:length(session_file_numbers)
            ...
            load(raw_data_filepath, 'allMUA','respCode');
            ...
            ReFactor = IdxRearrage(respCode, original_session_factor);
            ...
            elseif strcmpi(mua_lfp_idx,'MUA2') 
                trialdata = [];
                for i = 1:length(allMUA)
                    trialdata(i,:,:) = allMUA{i}(:,1:3000);
                end 
                neural_data_for_session = trialdata;
...
function [repeatnum_per_ori, ClusterData] = TrialDataRearrange(config, ReFactor, trialdata)
    ...
    for i = 1:length(uniqueChars)
        ...
        if strcmp(currentBlock,'SSGnv')
            for loc = 1:13
                ...
                for condi = 1:length(-1:2:17)
                    ...
                    data = trialdata(idx, 1:96, 1:3000);
                    ...
                    % filteredData = reshape(filteredData,[1,96,3000]);
                    ClusterData.SSGnv{1, loc, condi} = single(filteredData);
        ...
        elseif strcmp(currentBlock,'blank')
            idx = charIndices{i};
            ClusterData.blank = cat(1,ClusterData.blank,single(trialdata(idx,1:96,1:3000)));
        else
            condition = t.condition;
            for condi = 1:length(condition)
                idx1 = [currentBlockidx.Condition] == condition(condi);
                idx = charIndices{i}(idx1);
                data = trialdata(idx, 1:96, t.triallength);
                filteredData = data - mean(data(:,:,1:100),3);
                % filteredData = filtfilt(b, a, single(data)')'; % 滤波
                % filteredData = reshape(filteredData,[1,96,3000]);
...
function save_processed_block_data(config, processed_data)
    ...
    for i = 1:length(block_names)
        ...
        is_data_empty = false;
        if isstruct(data_to_save) && isfield(data_to_save, 'Data') && isempty(data_to_save(1).Data)
        ...
        if ~is_data_empty
            save_filename = sprintf('%sSSVEP1000hz_%s_%s_%s.mat', file_prefix, days_str, mua_lfp_str, current_block_name);
