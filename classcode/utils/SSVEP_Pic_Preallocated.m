...
function SSVEP_Pic_Preallocated(file_idx, MUA_LFP,label, label2)
...
for i = 1:length(file_idx)
    ...
    for cond = 1:num_conds
        ...
        pic_ori_matrix = cond_struct.Pic_Ori;
    ...
    % --- 2. 内存预分配 ---
    fprintf('    - 正在预分配内存...\n');
    SSVEP_PIC_DATA = cell(size(counter_matrix));
    ...
    for idx = 1:numel(SSVEP_PIC_DATA)
        if counter_matrix(idx) > 0
            SSVEP_PIC_DATA{idx} = zeros(counter_matrix(idx), num_channels, WINDOW_LENGTH, 'int16');
        end
    end
    ...
    for cond = 1:num_conds
        ...
        for trial = 1:size(trial_data, 1)
            ...
            for pic = 1:NUM_PICS
                window_start = BASELINE_END_MS + (pic - 1) * PIC_INTERVAL_MS + 1;
                window = window_start + WINDOW_RANGE;
                ...
                if is_ssg_case
                    ...
                    SSVEP_PIC_DATA{ori, pattern, location}(current_fill_idx, :, :) = corrected_data;
                    fill_idx_matrix(ori, pattern, location) = current_fill_idx + 1;
                else
                    current_fill_idx = fill_idx_matrix(ori, pattern);
                    SSVEP_PIC_DATA{ori, pattern}(current_fill_idx, :, :) = corrected_data;
    ...
    output_filename = sprintf('SSVEP_PIC_DATA_%s_%s_%s_%s.mat',label2, file_name(1:2), MUA_LFP, current_label);
    
    save(output_filename, 'SSVEP_PIC_DATA', '-v7.3');
    fprintf('    ✔ 已完成并保存: %s\n\n', output_filename);
