function h_fig = plot_pca_2d(traj_data, varargin)
% PLOT_PCA_2D 绘制2D神经群体轨迹
% 
% 输入:
%   traj_data: [nCond, nTime, 2] 数据矩阵 (由 compute_pca_trajectory 输出)
% 
% 可选参数 (Name-Value):
%   'TimeVec'    : 时间向量 [1, nTime] (默认: 1:nTime)
%   'CondColors' : [nCond, 3] 颜色矩阵 (默认: 自动生成)
%   'CondLabels' : {nCond, 1} 条件标签 (默认: {'Cond 1', 'Cond 2', ...})
%   'Title'      : 图像标题 (默认: '2D Neural Trajectories')
...
%   h_fig        : 图形句柄 (或坐标轴句柄)
    ...
    % --- 参数解析 ---
    p = inputParser;
    addRequired(p, 'traj_data', @(x) isnumeric(x) && ndims(x) == 3 && size(x, 3) >= 2);
    addParameter(p, 'TimeVec', [], @isnumeric);
    addParameter(p, 'CondColors', [], @isnumeric);
    addParameter(p, 'CondLabels', {}, @iscell);
    addParameter(p, 'Title', '2D Neural Trajectories', @ischar);
    ...
    for c = 1:nCond
        ...
        % 绘制轨迹
        plot(ax, x, y, 'Color', colors(c, :), 'LineWidth', p.Results.LineWidth, ...
        ...
        if p.Results.MarkEnd
            plot(ax, x(end), y(end), 's', 'MarkerSize', 8, ...
                'MarkerFaceColor', colors(c, :), 'MarkerEdgeColor', 'k', ...
                'HandleVisibility', 'off');
        end
        
        % (可选) 可以在轨迹上添加时间流动的箭头或渐变色，这里保持简洁
    end
