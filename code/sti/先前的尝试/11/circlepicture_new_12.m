...
figure_path='D:\Desktop\1';
%% 设定参数
model = 2;           %光栅排列方式：1-环形；2-双环形；3-3*3方形；4-4*4方形；

%% 单个光栅基本参数
numCycles = 3;
gaborDimPix = 10;        %光栅的绘制范围
freq = numCycles / gaborDimPix;%光栅空间频率
sigma = gaborDimPix / 7; %高斯包络              
contrast = 0.9;
aspectRatio = 1.0;
phase = 0;
...
orientation_step =5; %光栅朝向的分辨率
figure_num=180./orientation_step;
...
%       newgaborMatrix_in = gaborsRects_in(:,randomSelectionL_in);
    ...
    elseif model == 1
        ...
        gaborsRects = calculateCircleBoundaries(circleone, 300);
...
%     绘制图片
    filename = sprintf('circle_one_%d.jpeg', i*orientation_step);
    fullpath = fullfile(figure_path, filename);
