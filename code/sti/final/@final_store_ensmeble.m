% 文件路径
clear
for gsq=1:9  %需要生成几个GSQ文件
    
filename = 'z144repeats45.GSQ';

load(['random_sequence_all',num2str(gsq),'.mat'])
% 加载文本文件
...
for line_idx = 1:size(numeric_lines,2)-1
    ...
    % 修改第 10 列的值（假设从1开始计数）
    values{38} = [num2str(random_sequence_all(line_idx)),';'];  % 将第 10 列的值修改为 100
...
% 保存修改后的数据到新文件
new_filename = ['z144s',num2str(gsq),'_EC_20240724.GSQ'];
